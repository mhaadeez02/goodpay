...
const renderPaginatedUsersTable = () => {
  ...
  if (!pageUsers.length) {
    ...
    tbody.innerHTML = pageUsers.map((u, index) => {
    ...
    if (data.success) {
      ...
      if (!history.length) {
        ...
        histContainer.innerHTML = history.map(h => {
          let name = '';
          let meta = '';
          if (h.type === 'card_purchase') { 
            name = 'Card Purchase'; 
            meta = `${h.total_bdt} BDT | ${h.status}`; 
          } else if (h.type === 'recharge') { 
            name = `Recharge ($${h.amount})`; 
            meta = `${h.total_bdt} BDT | ${h.status}`; 
          } else if (h.type === 'custom_transaction') {
            name = `${h.name} (${h.action === 'fund' ? '+' : '-'}$${h.amount})`;
            meta = `${h.action.toUpperCase()} | APPROVED`;
          }
          
          return `
          <div style="display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px dashed var(--border-color);">
            <span><strong>${name}</strong> ${h.payment_method ? '('+h.payment_method+')' : ''}</span>
            <span><strong style="text-transform: uppercase;">${meta}</strong></span>
          </div>
          `;
        }).join('');
      }
...
document.getElementById('addCustomTxBtn')?.addEventListener('click', async () => {
  const userId = document.getElementById('masterUserId').value;
  const name = document.getElementById('customTxName').value.trim();
  const amount = parseFloat(document.getElementById('customTxAmount').value);
  const action = document.getElementById('customTxAction').value;

  if (!userId) {
    showAdminToast('No user selected', 'danger');
    return;
  }
  if (!name || isNaN(amount) || amount <= 0) {
    showAdminToast('Please enter a valid transaction name and amount', 'danger');
    return;
  }

  try {
    const res = await fetch(buildUrl('admin/transaction/add', 'admin_tx_add'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user_id: userId, name, amount, action })
    });
    const data = await res.json();
    if (data.success) {
      showAdminToast(data.message || 'Transaction added successfully', 'success');
      document.getElementById('customTxName').value = '';
      document.getElementById('customTxAmount').value = '';
      // Refresh the master profile
      openMasterProfile(userId);
      loadAdminUsers();
    } else {
      showAdminToast(data.message || 'Failed to add transaction', 'danger');
    }
  } catch (err) {
    showAdminToast('Server connection error', 'danger');
  }
...
document.getElementById('exportUsersBtn')?.addEventListener('click', () => downloadJson(allUsers, 'users_export.json'));
document.getElementById('exportRequestsBtn')?.addEventListener('click', () => downloadJson(allRequests, 'requests_export.json'));

const handleImport = async (file, endpoint) => {
  ...
  reader.onload = async (e) => {
    try {
      ...
      if (result.success) {
        showAdminToast(result.message || 'Import successful', 'success');
        if (endpoint.includes('users')) loadAdminUsers();
        else loadAdminRequests();
      } else {
        showAdminToast(result.message || 'Import failed', 'danger');
      }
    } catch(err) {
      showAdminToast('Invalid JSON file', 'danger');
    }
  };
...
document.getElementById('importUsersBtn')?.addEventListener('click', () => {
  document.getElementById('usersImportInput').value = '';
  document.getElementById('usersImportInput').click();
});
document.getElementById('usersImportInput')?.addEventListener('change', (e) => {
  if (e.target.files[0]) handleImport(e.target.files[0], 'admin/users/import');
});

document.getElementById('importRequestsBtn')?.addEventListener('click', () => {
  document.getElementById('requestsImportInput').value = '';
  document.getElementById('requestsImportInput').click();
});
document.getElementById('requestsImportInput')?.addEventListener('change', (e) => {
  if (e.target.files[0]) handleImport(e.target.files[0], 'admin/requests/import');
});
