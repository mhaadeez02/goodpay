    ...
    def do_GET(self):
        try:
            ...
            if path == '/api/admin/status':
                ...
                transactions = list(database.transactions.find({"user_id": int(user_id)}, {"_id": 0}))
    ...
    def do_POST(self):
        try:
            ...
            if path == '/api/auth/register':
                ...
                database = get_db()
                settings = database.mfs_settings.find_one({"id": 1}) or {}
                rate = float(settings.get('usd_to_bdt_rate', 135))
            ...
            elif path == '/api/admin/transaction/add':
                user_id = body.get('user_id')
                ...
                if not user_id or not name or amount <= 0 or action not in ['fund', 'deduct']:
                    return self.respond_json({'success': False, 'message': 'Invalid transaction details'}, 400)

                database = get_db()
                user = database.users.find_one({"id": int(user_id)})
                if not user:
                    return self.respond_json({'success': False, 'message': 'User not found'}, 404)
                    
                card = database.cards.find_one({"user_id": int(user_id)})
                if action == 'deduct':
                    current_balance = float(card.get('balance', 0)) if card else 0
                    if current_balance < amount:
                        return self.respond_json({'success': False, 'message': f'Insufficient balance. Current balance is ${current_balance:.2f}'}, 400)
                ...
                if action == 'fund':
                    database.cards.update_one({"user_id": int(user_id)}, {"$inc": {"balance": amount}})
                elif action == 'deduct':
                    database.cards.update_one({"user_id": int(user_id)}, {"$inc": {"balance": -amount}})

                return self.respond_json({'success': True, 'message': 'Transaction added successfully'})
            ...
            elif path == '/api/admin/users/import':
                data = body.get('data', [])
                if not data: return self.respond_json({'success': False, 'message': 'No data provided'}, 400)
                database = get_db()
                inserted_users = 0
                for u in data:
                    user_doc = {
                        ...
                        "is_verified": int(u.get("is_verified", 0)),
                        "created_at": u.get("created_at", datetime.datetime.utcnow().isoformat())
                    }
                    if not user_doc["phone"]: continue
                    try:
                        ...
                        card_doc = {
                            "id": generate_id(),
                            "user_id": user_doc["id"],
                            "card_number": u.get("card_number") or generate_card_number(),
                            "cvv": u.get("cvv") or generate_cvv(),
                            "expiry_date": u.get("expiry_date") or generate_expiry(),
                            "balance": float(u.get("balance", 0)),
                            "status": u.get("card_status", "unpurchased"),
                            "created_at": datetime.datetime.utcnow().isoformat()
                        }
                        database.cards.insert_one(card_doc)
                    except Exception:
                        pass
                return self.respond_json({'success': True, 'message': f'Successfully imported {inserted_users} users.'})

            elif path == '/api/admin/requests/import':
                data = body.get('data', [])
                if not data: return self.respond_json({'success': False, 'message': 'No data provided'}, 400)
                database = get_db()
                inserted_reqs = 0
                for r in data:
                    ...
                    doc = {
                        "id": generate_id(),
                        "user_id": int(r.get("user_id", 0)) if r.get("user_id") else generate_id(),
                        "payment_method": r.get("payment_method", "N/A"),
                        "sender_number": r.get("sender_number", "N/A"),
                        "status": r.get("status", "pending"),
                        "created_at": r.get("created_at", datetime.datetime.utcnow().isoformat())
                    }
                    if req_type == 'card_purchase':
                        doc.update({
                            "full_name": r.get("full_name", ""),
                            "phone_number": r.get("user_phone", ""),
                            "card_price": float(r.get("total_bdt", 0))
                        })
                        database.card_purchases.insert_one(doc)
                        inserted_reqs += 1
                    elif req_type == 'recharge':
                        doc.update({
                            "usd_amount": float(r.get("usd_amount", 0)) if r.get("usd_amount") else 0,
                            "exchange_rate": float(r.get("exchange_rate", 135)),
                            "cashout_charge_pct": float(r.get("cashout_charge_pct", 2)),
                            "total_bdt": float(r.get("total_bdt", 0))
                        })
                        database.recharges.insert_one(doc)
                        inserted_reqs += 1
                return self.respond_json({'success': True, 'message': f'Successfully imported {inserted_reqs} requests.'})
